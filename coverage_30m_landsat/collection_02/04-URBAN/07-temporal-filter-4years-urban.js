...
var Filter_3years = ee.Image('projects/mapbiomas-chile/assets/LULC/COLLECTION-02/CLASSIFICATIONS/classification-infrastructure/urban-temporal-filter-3-'
...
var filtered = window4years(Filter_3years, 68); 
filtered = window4years(filtered, 24);
...
filtered = window4years(filtered, 68); 
filtered = window4years(filtered, 24);
...
Map.addLayer(class_outTotal, visClass, 'filtered_ 4years', false);
...
Export.image.toAsset({
    "image": prueba,
    "description": 'urban-temporal-filter-4-'+ecorregionId.toString() + '-'+ ecorregionxregionId +'-'+version.output,
    "assetId": assetClass + 'urban-temporal-filter-4-' +ecorregionId.toString() + '-'+ ecorregionxregionId +'-'+version.output,
    "scale": 30,
    "pyramidingPolicy": {
        '.default': 'mode'
    },
