...
var Filter_4years = ee.Image('projects/mapbiomas-chile/assets/LULC/COLLECTION-02/CLASSIFICATIONS/postproceso/temporal-filter-4-'
...
var Filter_3years = ee.Image('projects/mapbiomas-chile/assets/LULC/COLLECTION-02/CLASSIFICATIONS/postproceso/temporal-filter-3-'
                                                      +ecorregionId.toString()+'-'+ecorregionxregionId.toString()+'-'+version.input.toString());
var spatialFilter = ee.Image('projects/mapbiomas-chile/assets/LULC/COLLECTION-02/CLASSIFICATIONS/classification-final/spatial-Filter-3')
...
var filtered = window5years(Filter_4years,  3)
filtered = window5years(filtered, 66)
filtered = window5years(filtered, 9)
filtered = window5years(filtered, 18)
filtered = window5years(filtered, 11)
filtered = window5years(filtered, 63)
filtered = window5years(filtered, 61)
filtered = window5years(filtered, 23)
filtered = window5years(filtered, 34)
filtered = window5years(filtered, 33)
...
Map.addLayer(Filter_3years, visClass, 'Filter_3years', false);
...
Export.image.toAsset({
    "image": prueba,
    "description": 'temporal-filter-5-'+ecorregionId.toString() + '-'+ ecorregionxregionId +'-'+version.output,
    "assetId": assetClass + '/temporal-filter-5-' +ecorregionId.toString() + '-'+ ecorregionxregionId +'-'+version.output,
    "scale": 30,
    "pyramidingPolicy": {
        '.default': 'mode'
    },
