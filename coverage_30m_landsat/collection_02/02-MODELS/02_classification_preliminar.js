...
var ecoregionId = 'E6S1'; // define la ecoregión
var version_output = 2; //y la versión de la clasificación 
...
var assetSamples = 'projects/mapbiomas-chile/assets/LULC/COLLECTION-02/SAMPLES/BASE/'+ecoregionId.toString(); //Directorio de entrada y salida para las muestras
var assetClass = 'projects/mapbiomas-chile/assets/LULC/COLLECTION-02/CLASSIFICATIONS/classification-preliminar-'+ecoregionId.toString(); //ImageCollection para guardar clasificación preliminar 
...
var classWeights = [
    ...
    [29, 0.1], // Afloramiento Rocosos
   // [24, 1], // Infraestructura Urbana
    [63, 0.3], // Estepa
    [61, 0.2], // Salares
    [25, 1], // Otras Areas sin Vegetacion
    [33, 0.2], // Rios, Lagos y Oceanos
    [34, 0.1], // Nieve y Hielo
//  [9, 0],  // Silvicultura
...
var aditionalSamplesPoints = trainingPoints.merge(validationPoints);
//
...
years.forEach(
    function (year) {
        ...
        var trainedSamples = ee.FeatureCollection(
            assetSamples + '/samples-points-' + ecoregionId.toString()+ '-' + year.toString() + '-' + version_samples);
...
// Visualizar
      ...
      //print(class01_img)
Map.addLayer(classE4, visClass, year + '-Mosaico-'  + 'class-E4', false);
Map.addLayer(classE2, visClass, year + '-Mosaico-'  + 'class-E2', false);
Map.addLayer(classE3, visClass, year + '-Mosaico-'  + 'class-E3', false);
Map.addLayer(class01_img, visClass, 'Colección 01 2022', false);

        Map.addLayer(mosaicYear, visMos, year + '-Mosaico', false);
        Map.addLayer(classified, visClass, year + '-Mosaico-'  + 'class', false);
    ...
    // visualize points using mapbiomas color palette
        ...
        var pointsName = 'samples-points-' + ecoregionId.toString()+'-' + year.toString() + '-aditional-' + version_output;
...
Export.image.toAsset({
    ...
    description: 'classification-preliminar-' + ecoregionId + '-' + year + '-v' + version_output,
    assetId: assetClass + '/classification-preliminar-' + ecoregionId + '-' + year + '-v' + version_output,
...
Map.addLayer(salares, {color:'#ff33f6'}, 'Inventario de Salares');
Map.addLayer(humedales, {color:'#3352ff'}, 'humedales');
//----------------------------------------------------------------------------------


//----------------------------------------------------------------------------------
/*
//Convertir en stack de imagenes
...
Export.image.toAsset({
    ...
    "description": 'chile-ecorregion-E'+ecoregionId.toString()+subecoregionId.toString()+ '-' + version_output,
    "assetId": assetClass + '/chile-ecorregion-E' + ecoregionId.toString()+subecoregionId.toString() + '-' +version_output,
    "scale": 30,
    "pyramidingPolicy": {
        '.default': 'mode'
    },
